...
#define NT_DEVICE_NAME      L"\\Device\\SIOCTL_TP"
#define DOS_DEVICE_NAME     L"\\GLOBAL??\\ThreadpoolDriver"

#define NT_DEVICE_NAME_TP_DRIVER L"\\Device\\MYTPDRV"
...
MyCreateClose(
    ...
    DbgPrintEx(
        DPFLTR_IHVDRIVER_ID,
        DPFLTR_ERROR_LEVEL,
        "Threadpool Driver create or close triggered %d\r\n",
        180
    );
...
DriverControl(
    ...
    DbgPrintEx(
        DPFLTR_IHVDRIVER_ID,
        DPFLTR_ERROR_LEVEL,
        "[Level 2] Hello from Threadpool Driver. NOOT NOOT!\r\n"
    );
    ...
    case IOCTL_SIOCTL_METHOD_BUFFERED_TP_INIT:
        DbgPrintEx(
            DPFLTR_IHVDRIVER_ID,
            DPFLTR_ERROR_LEVEL,
            "[Called IOCTL_SIOCTL_METHOD_BUFFERED_TP_INIT]\r\nThreadpool init was called\r\n"
        );
...
)
{
    ...
    DbgPrintEx(
        DPFLTR_IHVDRIVER_ID,
        DPFLTR_ERROR_LEVEL,
        "[TPDriver] Hello from driver load %d\r\n",
        150
    );
    __debugbreak();
}
...
)
{
    ...
    RtlInitUnicodeString(
        &ntDeviceName,
        NT_DEVICE_NAME_TP_DRIVER
    );

    ntStatus = IoCreateDevice(
        DriverObject,               // Our Driver Object
        0,                          // We don't use a device extension
        &ntDeviceName,              // Device name "\Device\MYTPDRV"
    ...
    if (!NT_SUCCESS(ntStatus)) {
        DbgPrintEx(
            DPFLTR_IHVDRIVER_ID,
            DPFLTR_ERROR_LEVEL,
            "[TPDriver] Couldn't create the device object\r\n"
        );
        return ntStatus;
    }
